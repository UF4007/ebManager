...
- 出于数据结构的便捷性与稳定性考虑，单个eb::manager之内所有操作均线程不安全。多线程需要通过设计保证其线程安全性。
...
- 单个eb::manager之内，所有操作均线程不安全，包括序列化、反序列化、构造、析构等。
- 若有两个eb::base属于同一个eb::manager，即使他们之间没有指针关联，在多线程下对他们同时进行序列化、反序列化等操作，也不是线程安全的。
- 不同的eb::manager之间，若不涉及出入口操作，是线程安全的。
